                ...
                if (apiMessages[i].role === 'user') {
                    ...
                    if (aiModel === 'glm-4-6' && isPseudoThinkingModeEnabled && !isAutoGenerated) {
                        pseudoThinkingInstruction = sessionLang === 'ja'
                            ? `\n\n【指示】\nユーザーからのメッセージに対応するために、事前に段階的かつ論理的に対応方法について思考しましょう。\n思考プロセスをテキスト出力し、思考が完了したら最後に「[THINKING_COMPLETE]」と出力してください。\n※ユーザーに思考が行われていることを確認できるように、必ず「[THINKING_COMPLETE]」を出力する前に、何かしらの具体的な思考内容のテキストを出力してください。`
                            : `\n\n[Instruction]\nTo respond to the user's message, please think step-by-step and logically beforehand.\nOutput your thought process, and once your thinking is complete, output "[THINKING_COMPLETE]" at the end.\n* To ensure the user can confirm your thinking process, you MUST output some concrete thought text BEFORE outputting "[THINKING_COMPLETE]".`;
                    }
            ...
            try {
                while (loopCount < MAX_LOOPS) {
                    ...
                    if (isIteratingPseudoThought) {
                        ...
                        displayPseudoThought = activePseudoThought.replace(/\[THINKING_COMPLETE\]/g, "").trim();
                    ...
                    if (finalFunctionCall) {
                        ...
                        if (aiModel === 'glm-4-6' && isPseudoThinkingModeEnabled && !isAutoGenerated) {
                            // Turn pseudo-thinking back on for the next loop to digest the tool result
                            isIteratingPseudoThought = true;
                            const postToolThinkInstruction = sessionLang === 'ja'
                                ? `\n\n【指示】\n上記のツール実行結果（あるいはバックグラウンドでの処理完了）を受け、ユーザーへの最終的な回答（本回答）を行う前に、再度どのように回答を構成するか段階的かつ論理的に思考してください。\n思考プロセスをテキスト出力し、思考が完了したら最後に「[THINKING_COMPLETE]」と出力してください。\n※ユーザーに思考が行われていることを確認できるように、必ず「[THINKING_COMPLETE]」を出力する前に、何かしらの具体的な思考内容のテキストを出力してください。`
                                : `\n\n[Instruction]\nBased on the tool execution result above (or background processing completion), before providing your final response to the user, please think step-by-step logically again about how to construct your answer.\nOutput your thought process, and once your thinking is complete, output "[THINKING_COMPLETE]" at the end.\n* To ensure the user can confirm your thinking process, you MUST output some concrete thought text BEFORE outputting "[THINKING_COMPLETE]".`;
                        ...
                        const actualTextContent = textBeforeTool || finalText;
                        // For the final message (if any), we also want to show the thinking if it wasn't already consumed by a tool call early breakout
                        const rawThoughtForSummary = displayPseudoThought || accumulatedThought || "";
