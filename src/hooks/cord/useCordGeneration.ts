                    ...
                    if (finalFunctionCall) {
                        ...
                        if (aiModel === 'glm-4-6' && isPseudoThinkingModeEnabled && !isAutoGenerated) {
                            ...
                            const postToolThinkInstruction = sessionLang === 'ja'
                                ? `\n\n【指示】\n上記のツール実行結果（あるいはバックグラウンドでの処理完了）を受け、ユーザーへの最終的な回答（本回答）を行う前に、再度どのように回答を構成するか段階的かつ論理的に思考してください。思考プロセスをテキスト出力し、思考が完了したら最後に「[THINKING_COMPLETE]」と出力してください。`
                                : `\n\n[Instruction]\nBased on the tool execution result above (or background processing completion), before providing your final response to the user, please think step-by-step logically again about how to construct your answer. Output your thought process, and once your thinking is complete, output "[THINKING_COMPLETE]" at the end.`;
