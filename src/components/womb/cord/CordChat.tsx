...
export const CordChat: React.FC<CordChatProps> = ({
    lang, currentStoryId, content, triggerSave, showDebugInfo = false, apiKey, novelAIApiKey, aiModel,
    ...
    const handleSend = async () => {
        if (!inputValue.trim() || isTyping || !isCordStorageReady) return;

        // 1. Add User Message (Returns active sessionId)
        const currentInput = inputValue; // Capture locally
        setInputValue(''); // Clear aggressively
        const activeSessionId = await addMessage('user', currentInput, currentSessionId || undefined);

        // 2. Trigger AI Response
        if (activeSessionId) {
            generateAiResponse(activeSessionId, apiKey, novelAIApiKey, aiModel, getWombContext);
        }
    };
    ...
    useEffect(() => {
        const handleStartBackgroundMode = async (e: Event) => {
            ...
            const newSessionId = await addMessage('user', instructionMsg, currentSessionId || undefined, undefined, undefined, undefined, true);
            if (newSessionId) {
                generateAiResponse(newSessionId, apiKey, novelAIApiKey, aiModel, getWombContext);
            }
        ...
        const handleRequestWombGen = async () => {
    ...
    if (!isCordStorageReady) {
        return (
            <div style={{
                display: 'flex', justifyContent: 'center', alignItems: 'center',
                height: '100%', width: '100%', backgroundColor: 'transparent', color: '#38bdf8',
                flexDirection: 'column', gap: '1rem', fontFamily: 'monospace'
            }}>
                <div className="cord-loading-spinner" style={{
                    width: '30px', height: '30px', border: '2px solid rgba(56, 189, 248, 0.2)',
                    borderTopColor: '#38bdf8', borderRadius: '50%', animation: 'spin 1s linear infinite'
                }} />
                <div style={{ fontSize: '0.8rem' }}>INITIALIZING CORD DB...</div>
            </div>
        );
    }
