...
export const LorebookSidebar: React.FC<LorebookSidebarProps> = ({
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file && onImportAllEntities) {
            onImportAllEntities(file);
        }
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button
                        ...
                        disabled={!onImportAllEntities}
                        title={lang === 'ja' ? 'ZIPからEntityをインポート' : 'Import Entities from ZIP'}
                        style={{
                            background: 'transparent',
                            border: '1px solid rgba(56, 189, 248, 0.3)',
                            borderRadius: '4px',
                            color: '#38bdf8',
                            cursor: onImportAllEntities ? 'pointer' : 'default',
                            padding: '0.3rem',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            transition: 'all 0.2s',
                            opacity: onImportAllEntities ? 1 : 0.5
                        }}
                        onMouseEnter={e => {
                            if (onImportAllEntities) {
                    ...
                    <button
                        onClick={onDownloadAllEntities}
                        disabled={!onDownloadAllEntities}
                        title={lang === 'ja' ? '全てのEntityをZIPでダウンロード' : 'Download All Entities as ZIP'}
                        style={{
                            background: 'transparent',
                            border: '1px solid rgba(56, 189, 248, 0.3)',
                            borderRadius: '4px',
                            color: '#38bdf8',
                            cursor: onDownloadAllEntities ? 'pointer' : 'default',
                            padding: '0.3rem',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            transition: 'all 0.2s',
                            opacity: onDownloadAllEntities ? 1 : 0.5
                        }}
                        onMouseEnter={e => {
                            if (onDownloadAllEntities) {
                                e.currentTarget.style.backgroundColor = 'rgba(56, 189, 248, 0.2)';
                                e.currentTarget.style.color = '#fff';
                            }
                        }}
